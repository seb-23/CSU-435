	...
	public static class CountReducer extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
			...
			for (Text value : values) {
				...
				String c = value.toString() + " " + key.toString();
				if (distinctNodes.count(c) > 0) {
					...
					if ( s.length() > r.length() || (s.length() == r.length() && s.compareTo(r) > 0) ) {
						...
						distinct.add(key.toString() + " " + value.toString());
					}
				}
			}
		}
		...
		protected void cleanup(Context context) throws IOException, InterruptedException { 
			...
			ArrayList<String> finale = new ArrayList<String>(distinct);
