...
public class Task2_InDegree {
	...
	public static class CountReducer extends Reducer<Text, Text, Text, Text> {
		...
		private Multiset<String> distinctNode;
	
		@Override
		protected void setup(Context context) {
			distinctNodes = HashMultiset.create();
			distinctNode = HashMultiset.create();
		}
		...
		protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
			...
			for (Text value : values) {
				String c = key.toString() + " " + value.toString();
				if (distinctNode.count(c) == 0) {
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(Task2_InDegree.class);
		job.setMapperClass(Task2_InDegree.TokenizerMapper.class);
		job.setReducerClass(Task2_InDegree.CountReducer.class);
